...
function network_write(data, socket = oClient.sock) {
	...
	if (SOCKET_TYPE == SOCKET_TYPES.TCP) {
		...
		// send!
		status = network_send_raw(socket, new_buff, size+4)
		...
		status = network_send_raw(socket, buff, size, network_send_binary)
	}
...
///@arg data
///@arg *socket
function send(data, socket = oClient.socket) {
